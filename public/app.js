...
function showAttendedInfo(rec) {
  const btn       = document.getElementById('checkin-btn');
  const infoEl    = document.getElementById('attended-info');
  const photoEl   = document.getElementById('attended-photo');
  const noPhotoEl = document.getElementById('attended-no-photo');
  const timeEl    = document.getElementById('attended-time');
  const locEl     = document.getElementById('attended-loc');

  btn.classList.add('hidden');
  infoEl.classList.remove('hidden');

  setText('attended-time', rec.time ? rec.time.slice(0, 5) : '–');  // HH:MM
  setText('attended-loc',  rec.location || 'Location not recorded');

  if (rec.photo_path) {
    photoEl.src = rec.photo_path;
...
function hideAttendedInfo() {
  document.getElementById('checkin-btn').classList.remove('hidden');
  document.getElementById('attended-info').classList.add('hidden');
}
...
async function loadMemberTasks() {
  try {
    ...
    window._memberTasksCache = tasks;
    ...
    <div class="task-item ${isDone ? 'done' : ''}">
      ...
      <div class="task-body">
        ...
        <div class="task-meta">
          ...
          ${t.completed_at ? `<span class="task-meta-done">✓ Done ${fmtDateTime(t.completed_at)}</span>` : ''}
        </div>
      </div>
      <div class="member-task-actions">
        <span class="task-status-badge ${t.status}">${fmtStatus(t.status)}</span>
        ${!isDone ? `
          <button class="task-btn-progress ${t.status === 'in-progress' ? 'active' : ''}"
            onclick="memberSetStatus(${t.id},'${t.status}','in-progress')"
            title="Mark In Progress">▶</button>
          <button class="task-btn-done"
            onclick="memberMarkDone(${t.id})"
            title="Mark Done">✓</button>
        ` : `
          <button class="task-btn-reopen"
            onclick="memberSetStatus(${t.id},'${t.status}','pending')"
            title="Reopen">↺</button>
        `}
      </div>
    </div>`;
...
// Member: mark a task done with confirmation
window.memberMarkDone = function(id) {
  const task = (window._memberTasksCache || []).find(t => t.id === id);
  const title = task ? `"${task.title}"` : 'this task';
  confirmAction(
    `Mark ${title} as Done?`,
    'The exact completion time will be recorded and saved.',
    '✓',
    async () => {
      try {
...
// Member: set any non-done status directly (no confirmation needed)
window.memberSetStatus = async function(id, current, newStatus) {
  if (newStatus === current) return;
  try {
    await api('PUT', `/api/tasks/${id}`, { status: newStatus });
    toast(`Marked as ${fmtStatus(newStatus)}`, 'success');
    loadMemberTasks();
  } catch (e) { toast(e.message, 'error'); }
};
...
window.memberTab = function(tab, btn) {
  ...
  const map = { home:'attendance-status-card', tasks:'member-tasks-list' };
...
window.submitCheckin = async function() {
  ...
  try {
    ...
    loadMemberStatus(); loadMemberStats();
  } catch (err) {
    ...
    <div class="task-item ${isDone ? 'done' : ''}" onclick="openTaskDetail(${t.id})">
      ...
      <div class="task-body">
        ...
        <div class="task-meta">
          ...
          ${t.completed_at ? `<span class="task-meta-done">✓ ${fmtDateTime(t.completed_at)}</span>` : ''}
        </div>
      </div>
      <span class="task-status-badge ${t.status}">${fmtStatus(t.status)}</span>
